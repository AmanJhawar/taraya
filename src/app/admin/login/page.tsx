...
export default function AdminLogin() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-band py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full bg-field p-10 rounded-xl shadow-[0_8px_24px_rgba(35,31,29,0.06)] border border-line">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div>
            ...
            <input
              type="email"
              {...register('email')}
              className={`w-full px-4 py-3 rounded-lg border focus:border-ink focus:ring-4 focus:ring-ink/10 transition-[border-color,box-shadow] duration-200 ${errors.email ? 'border-ink' : 'border-line'}`}
          ...
          <div>
            ...
            <input
              type="password"
              {...register('password')}
              className={`w-full px-4 py-3 rounded-lg border focus:border-ink focus:ring-4 focus:ring-ink/10 transition-[border-color,box-shadow] duration-200 ${errors.password ? 'border-ink' : 'border-line'}`}
              placeholder="••••••••"
            />
